...
template<typename T_, typename C_ = less <typename T_::value_type>> // Default comparator is less than (<)
class CIndex {
public:
    CIndex() = default;

    CIndex(const T_ & t, const C_ & comparator = C_ ()) : target(t), cmp(comparator){}; // If comparator is not provided, set it to default C_ type.
    set<size_t> search(T_ search) {
        set<size_t> result;
        size_t idx = 0;
        bool found = false;
        if (search.size() == 0) {
        ...
        for (auto targetIt = target.begin(); targetIt != target.end(); targetIt++) {
            ...
            for (auto searchIt = search.begin(); searchIt != search.end(); searchIt++) {
                ...
                if (temp == target.end() || ((cmp(*temp, *searchIt)) || (cmp(*searchIt, *temp)))) {
                    found = false;
                    break;
                }
                temp++;
