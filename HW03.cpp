...
class CDate
{
  ...
  public:
    ...
    friend ostream & operator<< (ostream & os, CDate & date) {
      os << date.storedDate.tm_year + 1900 << '-' << setw(2) << setfill('0') << date.storedDate.tm_mon + 1 << '-' << setw(2) << setfill('0') << date.storedDate.tm_mday;
      return os;
    }
    ...
    friend istream & operator>> (istream & in, CDate & date) {
      ...
      if (!valiDate(y, m, d)) {
        ...
        date.storedDate.tm_mday = d;
        date.storedDate.tm_mon = m;
        date.storedDate.tm_year = y;
        return in;
      }
    }
};
